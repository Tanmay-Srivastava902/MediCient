...
def insert_record(cursor:MySQLCursorAbstract,table,column_order:tuple,values_list:list[tuple])->None:
    ...
    try:
        ...
        raise errors.RecordError(f'could not insert record: {str(e).strip()}')
...
def update_record(
    ...
    try:
        ...
        raise errors.RecordError(f'could not update record: {str(e).strip()}')
    except (InterfaceError,ProgrammingError) as e :
        raise errors.DBConnectionError(f'could not connect : {str(e).strip()}')
    except Error as e : 
        raise errors.ExecutionError(f'Could not execute query : {str(e).strip()}')
...
#     conn = connect(host = 'localhost',user='root',password='SecurePass@1201',db='information_schema')
    ...
    # delete_record(cursor:MySQLCursorAbstract, 'user',where_clause=[{'uid' : 7},{'uid': 15}],with_or=True)# deleted both 7 and 15
    # insert_record(cursor:MySQLCursorAbstract,'user',('uname','uid','uaddress'),[('tanmay',10,'nyaipur'),('shruti',15,'jaipur')]) # inserted 2 records in table users
    # insert_record(cursor:MySQLCursorAbstract,'user_test',('uname','uid'),[('tanmay',10,),('shruti',15)]) # inserted 2 record in table user_test (leaving tid to A_I)
    ...
    # select_record(cursor:MySQLCursorAbstract,'user',['uname','uaddress'],where_clause=[{'uaddress':'nyaipur'},{'uname':'d\\_%'}],with_or=False)
    ...
    # select_record(cursor:MySQLCursorAbstract,'user',['uname','uaddress'],where_clause=[{'uaddress':('nyaipur','wrong')},{'uname':'t%'}],with_or=True,order_by_column='uid',order='desc',offset=3)
    # select_record(cursor:MySQLCursorAbstract,'user',['uname','uaddress'],where_clause=[{'uaddress':('nyaipur','wrong')},{'uname':'t%'}],with_or=True,order_by_column='uid',order='desc',limit=2 , offset=3)
    # select_record(cursor:MySQLCursorAbstract,'user',['*'],where_clause=[{'uid': 'between 10 and 17' }],with_or=True,order_by_column='uid',order='desc',limit=5 , offset=2)
    # select_record(cursor:MySQLCursorAbstract,'TABLE_CONSTRAINTS',['*'],[{'CONSTRAINT_NAME':'fk_user_patient'},{'TABLE_NAME':'patient'}])
